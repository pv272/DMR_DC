...
- 33543 WAF045, Blue Moon evictee day 16 post eviction because 33543 (Day 12 post eviction) seem not to have been exported. 
...
#The Inventory info may not be the latest one
...
#Seems that 649 samples must be analyzed
...
Info would have to be added manually in the relevant table that lead to litter Info if I want these to be returned in SampleList
...
##Slow pre first and slow none 

Slow pre-first and slow none are being put together. This is a total of 4 groups:

Guinness: 
Corbeau: Parturition day. 
WicklowWolf (Slow None). Queen not obv pregnant or lact at eviction

In the 3 cases, the queen activated very early within the first month or so. 


Queen always ended up being evicted after 

```{r}

Slow_PreFirst_Check <- Eviction_RapidConc_Out_RelCol %>% 
  filter(Sampling_Cat %in% c("Slow_PreFirst", "Slow_None")) %>% 
  arrange(ExperimentalUnit,
          EvictionStatus,
          UrineNumber)
View(Slow_PreFirst_Check)

#CSV
write.csv(Slow_PreFirst_Check,"Slow_PreFirst_Check_20240403.csv",row.names = FALSE)


#I will use this as extraction sequence too
...
##Step 4: No eviciction and Rapid_Conc

```{r Freezer Out}

unique(PostPairing$Sampling_Cat)

#All post-pairing samples, except RapidConc
Eviction_RapidPre_NoEv <- PostPairing %>%
  
  #RETAIN ALL SAMPLING CAT EXCEPT RAPID CONC AND NONE 
  filter(Sampling_Cat %in% c("NoEviction","Rapid_Conc")) %>% 
...
View(Eviction_RapidPre_NoEv)
#222 samples 


#Check
View(Eviction_RapidPre_NoEv_Out %>% 
       filter(UrineIsol_DayDiff <= 0))
#All Good


#Number of No + Rapid_Conc samples present at the NPAC
#To get box and position to get samples out of the freezer 
Eviction_RapidPre_NoEv_FreezerOut <- Eviction_RapidPre_NoEv %>% 
...
View(Eviction_RapidPre_NoEv_FreezerOut)
#193 unique samples


#Write csv
#The list contains the duplicate
write.csv(Eviction_RapidPre_NoEv_FreezerOut,"Eviction_RapidPre_NoEv_FreezerOut_ToPrint_20240405.csv",row.names = FALSE)
...
#To place in correct sequence of extraction 
Eviction_RapidPre_NoEv_Out_RelCol <- Eviction_RapidPre_NoEv %>% 
  ...
  #JOIN TO BOX POSITION AFTER SAMPLES TAKEN OUT OF FREEZER
  #I take samples out of freezer in ascending order of box and position id
  left_join(.,Eviction_RapidPre_NoEv_FreezerOut %>% 
              select(UrineNumber,
                     NewBox,
                     NewPosition)) %>% 
...
View(Eviction_RapidPre_NoEv_Out_RelCol)
...
NoEviction_Check <- Eviction_RapidPre_NoEv_Out_RelCol %>% 
  filter(Sampling_Cat %in% c("NoEviction")) %>% 
  arrange(ExperimentalUnit,
          EvictionStatus,
          UrineNumber)
View(NoEviction_Check )

#Use the same for extraction seq 
...
RapidConc_Check <- Eviction_RapidPre_NoEv_Out_RelCol %>% 
  filter(Sampling_Cat %in% c("Rapid_Conc")) %>% 
  arrange(ExperimentalUnit,
          EvictionStatus,
          UrineNumber)
View(RapidConc_Check)

#Use the same for extraction seq 

#write csv

write.csv(RapidConc_Check,"RapidConc_Check.csv", row.names = FALSE)
...
Basically the one that should have been exported and were forgotten. Most important if they were part of the selection. 

Can they be replaced? I guess replace whenever possible 


#Visualize 

Can be done in a separate script data explor
To facilitate process perhaps best to visualize by sample reason as it may may things easier to visualize 
