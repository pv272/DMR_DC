...
Prepare DC data for further data exploration and statistical analyses
...
1) Run DB_Extract_YYYYMMDD
2) Run DP_IndividualLifeHistory_YYYYMMDD and DMR_Functions_YYYYMMDD
...
Dir_DC <- "C:/Users/laura/Dropbox/PhD_20150515/Research projects/DMR_HelperBreedingOpp/DC/DC_Analyses"
...
Get the hormone []. The csv is the output of the DC_DataPrep_Hormones R file. Currently the batch still contains Batch 49 and 50 and still contains duplicated samples ()
...
KingRemoval_Absence <- read.csv("KingRemovalDates_20240320.csv") %>% 
  select(NatalColony,
         KingRemovalDate) %>%
  rename(OriginalGroup = NatalColony) %>% 
  mutate(KingRemovalDate = dmy(KingRemovalDate)) %>% 
...
DC_Evictee <- DC_AnimalID %>% 
  ...
  filter(DateOfEviction < "2024-06-01") %>% 
...
DC_EvictionDates <- DC_EvictionInfo %>% 
  ...
  filter(EvictionDate < "2024-06-28")
...
DC_Parturition_Info_PostPair <- DC_Parturition_Info_All %>% 
...
View(DC_Parturition_Info_PostPair %>% 
       filter(PairingDate.x != PairingDate.y))

getwd()
#Write csv
write.csv(DC_Parturition_Info_PostPair,"DC_Parturition_Info_PostPair_20240827.csv",row.names = FALSE)
...
DC_Parturition_Info_DCFirst <- DC_Parturition_Info_PostPair %>% 
  group_by(AnimalID) %>% 
...
DC_Breeding_SameGroup <- DC_MultipleBreeding %>% 
  ...
  select(AnimalID) %>% 
  left_join(., DC_Parturition_Info_PostPair)
...
#Latest Parturition (pre-pairing)

Technically a pre-pairing info, but must be placed here in code as need some info from earlier 

Query the latest parturition in the original group. That will be useful for sample selection.

There should be only one single delay between latest parturition and isolation for females for each distinct Original group
...
LastParturition_OriginalGroup <- DC_PrePairingGroup %>%
  ...
  #ADD ISOLATION DATES 
  #WE ONLY INTERESTED IN FEMALES 
  left_join(., DC_IsolationDates %>% 
              distinct(AnimalID,PairingColony,IsolationDate) %>% 
              rename(ExperimentalGroup = PairingColony)) %>% 
  
  #ADD LITTER INFO OF THE ORIGINAL GROUP 
  left_join(., Litter_Info %>% 
              filter(Sex == "F") %>% 
              select(-AnimalID), by = c("OriginalGroup" = "ParturitionColony")) %>% 
  
  #ONLY RETAIN LATEST PARTURITION
  filter(ParturitionDate < IsolationDate) %>% 
  group_by(OriginalGroup, AnimalID) %>% 
...
DC_Info <- 
  ...
  #ADD LAST ORIGINAL GROUP ISOLATION DAY DIFF 
  mutate(LastOrGroupPartIsol_DayDiff = OriginalGroup_LastParturitionDate - IsolationDate) %>% 
  ...
  distinct()
#75 entries seems to be all good 
  

write.csv(DC_Info,"DC_Info_20250116.csv",row.names = FALSE)
...
#write.csv
write.csv(DC_Info_F,"DC_Info_F_20250116.csv",row.names = FALSE)
...
SamplingCat <- DC_Info_F %>% 
  ...
  mutate(Sampling_Cat = case_when(  ExperimentalGroup == "Tusker" ~ "Slow_PostFirst",
                                    Eviction_Cat == "Rapid" & Eviction_GS == "Conception" ~ "Rapid_Conc",
  ...
  mutate(Sampling_Cat = fct_relevel(Sampling_Cat, "Rapid_Conc", "Rapid_PreFirst", "Slow_Conc", "Slow_PreFirst", "Slow_PostFirst","Slow_None", "NoEviction")) %>% 
  
  arrange(Sampling_Cat)


write.csv(SamplingCat, "SamplingCat_20240815.csv", row.names = FALSE)
...
#write csv
write.csv(DC_Info_F,"DC_Info_F_20240816.csv",row.names = FALSE)
...
May have influenced the hormonal status of animals at the start of the experiment 
...
write.csv(DC_Info_Group,"DC_Info_Group_20250116.csv",row.names = FALSE)
...
#write csv
write.csv(DC_Urine_F, "DC_Urine_F_20240710.csv", row.names = FALSE)
...
Date_Eviction_ToAdd <- DC_EvictionDates %>%
  ...
  left_join(., DC_ExperimentalGroup %>% 
              distinct(AnimalID, ExperimentalGroup))
...
#write csv
write.csv(DC_ClosestWeight,"DC_ClosestWeight_20250116.csv",row.names = FALSE)
...
#Female only as there is more info in DC_Info_F 

#Female
DC_Info_Urine_F <- DC_Info_F %>%
  
  #ADD URINE 
  left_join(DC_Urine_F %>% 
              select(AnimalID,
                     UrineDate,
                     UrineNumber,
                     UrineDelay,
...
write.csv(DC_Info_Urine_F,"DC_Info_Urine_F_20240816.csv",row.names = FALSE)
