...
#Sample analysed in Batch 44-50
#Before redoing of 49 and 50 + additional samples
...
Determine differences between BF and NBF when there is no assymetry in breeding opportunity and determine whether ovulation occurs

Q1 - Do BF still ovulate in the absence of the king?
Q2 - DO NBF start ovulating in the absence of the BF (Sub treatment only - SubT VS QueenT)
Q3 - What are the hormonal differences between BF and NBF when assymetry in breeding opportunity is lifted


##Isolation samples

- 1 sample after two weeks 
- last 2 samples before pairing 

3 samples per female = 150 samples 
...
All samples that could have been analysed were analysed 

24 of the selected samples have not been analysed because they belonged to the samples of the 4 first ExperimentalUnit whose samples got defrosted

3 unit x 2 ID x 3 samples = 24 samples

No sample must be redone in that selection


```{r Analysis check} 

#Have all selected samples been extracted and quantified?
Sel_AnalysisCheck <- DC_Info_Urine_F %>%
  left_join(.,DC_Info_F %>% distinct(ExperimentalGroup,ExperimentalUnit)) %>% 
  #ADD SAMPLE INFO
  inner_join(.,Sel_Isol) %>%
  #ADD CONC 
  left_join(., DC_Conc_All, by=c("UrineNumber" = "SampleID")) %>% 
  filter(is.na(BatchID))
```

data exploration from individual graph suggest a mistake for ERF002 from Erotics. According to my files, this animal had samples 37356 and 37357 extracted and both supposedly refer to the same animal. 

This is a mistake I had noticed in the sample selection process and thought it was impossible since they were collected at the same time. Check the label on the sample but based on data 37356 should be excluded from dataset. Should check label on tubes 

```{r KGF002 Mistake}

ERF002_PrePair <- DC_Info_Urine_F %>%
  left_join(.,DC_Info_F %>% distinct(ExperimentalGroup,ExperimentalUnit)) %>% 
  #ADD SAMPLE INFO
  inner_join(.,Sel_Isol) %>%
  #ADD CONC 
  left_join(., DC_Conc_All, by=c("UrineNumber" = "SampleID")) %>% 
  #RETAIN KGF002
  filter(AnimalID == "ERF002",
         UrineIsol_DayDiff == 29) %>%
  select(AnimalID,
         UrineNumber,
         Hormone,
         Conc) %>% 
...
View(Urine_Collected %>% 
       filter(UrineNumber %in% c(37356,37357)))

```


##Pre-Isolation samples

The king was removed in the 12 groups of the queen treatment. 7 groups from helpers treatment were founded from the same group as above 

5 groups from helpers treatment were founded from groups not used for queen treatment.
1) Bennett
2) Curie
3) Goodall
4) Tinbergen
5) Watson. 
In these 5 groups, no king was removed and both breeders were in the group at isolation, suggesting all these groups could potentially breed.


Based on the overview of data: 

- Closest sample to pairing
- closest sample to 14 days => all BF had no more king and had their last parturition for quite a while. 

All the above must have been after the last parturition of the BF 
...
Sample 36751 from TIF039 -16 days isolation seem to not have exported and instead 36757 was exported. I realized this when extracting. Thus I replaced that sample with 36852 -9 days pre-isolation. 36751 will appear in the list returned below.

```{r Detail}

View(DC_Info_Urine_F %>% 
  filter(UrineNumber %in% c(36852, 36751)))

```

Sample 32233 (I assume) from SOF001 (Corbeau) 8 days before isolation seem to have been labelled 32230 by mistake. I realized this when extracting. I thus added 3233 (-8 days) sample manually as I was missing pre-isolation sample for that individual 

```{r Detail}

View(DC_Info_Urine_F %>% 
  filter(UrineNumber %in% c(32233, 32230)))

```

All animals have had at least one sample collected within 14 days of pairing

```{r Pre-Isol 14 days}


#Collected within 14 days of isolation
...
#Between 1 and 8 samples were collected in the last 14 days of sampling
...
Selection of last sample before isolation
...
Selection of closest sample to 14 days prior to isolation. I authorize samples collected within last 28 days of isolation. 
...
There are 12 animals that only have 1 selected samples 

```{r Selection check}

Sel_PreIsol <- bind_rows(Sel_PreIsol1,
          Sel_PreIsol14 ) %>% 
  distinct(UrineNumber)
View(Sel_PreIsol)
#88 samples
...
#Summary of all selected pre-isolation samples
Sel_PreIsol_Summary <- Sel_PreIsol %>% 
  ...
  summarise(Count = n(),
         Min = min(UrineIsol_DayDiff),
         Max = max(UrineIsol_DayDiff))
View(Sel_PreIsol_Summary)


#Animals with only 1 samples instead of 2
PreIsol_1Sample <- Sel_PreIsol_Summary %>% 
  filter(Count == 1)
View(PreIsol_1Sample)
#There are 12/50 animals that only have 1 samples 
#All of them are helpers


#All pre isolation sample collected from these animals

```


It seems that there are two selected samples that have not been analysed

- TIF039 (Asahi) -16day 36751: Seems not to have been exported and sample 36852 -9day was done as replacement

- KGF002 (Leffe) -13 day 36790: I don't know what is the reason for not doing it but seems that I have done 36670 (-15 day) instead

Thus we have analysed as per our plans. Only 12 animals have 1 sample instead of the 2

```{r Analyses check}

SelPreIsol_AnalysisCheck <- DC_Info_Urine_F %>%
  #ADD SAMPLE INFO
  inner_join(.,Sel_PreIsol) %>%
  #ADD CONC 
  left_join(., DC_Conc_All, 
by=c("UrineNumber" = "SampleID")) %>% 
  #REMOVE PAIR 1 to 4
  filter(!ExperimentalUnit %in% (1:4)) %>% 
  filter(is.na(BatchID))
View(SelPreIsol_AnalysisCheck)
```
...
No samples has been selected post-eviction yet so no need remove samples selected before
...
PostEviction_14 <- DC_Info_Urine_F %>%
  #FILTER
  filter(UrineEviction_DayDiff >= 14) %>% #Here I should have taken the closest to 14 instead of necessarily larger
  group_by(AnimalID) %>% 
  slice_min(UrineEviction_DayDiff) %>% 
  ungroup()
...
The list also does not return samples from SOF006 from Firestone for which I had added the date of parturition manually (she died when she gave birth). The 3 missing samples (34171,34256,34348) thus do not appear in the selection and will be added manually in the Extraction List in  
...
FirstConc <- DC_Info_Urine_F %>% 
  ...
  #RETAIN FIRST LITTER 
  filter(NextLitter_IndCount_Colony == 1) %>%  #This cause Firestone samples not to be returned because DOB had been added manually as she died when she gave birth and there is no litter info in DB 
...
#SOF006 from Firestone has no sample (Rapid_Pre) why?
