...
ClosestWeight_Milestone <- DC_Info_ClosestWeight %>% 
  ...
  #ADD HEAVIER REF 
  left_join(.,Isolation_HeavierIsolation) %>% 
...
HeavierFocal_WithNa <- ClosestWeight_Milestone %>% 
...
#Compute weight difference 
HeavierIsol_WithNa <- ClosestWeight_Milestone %>% 
...
#Compute weight difference 
BM_EvictorFocal_WithNa <- ClosestWeight_Milestone %>% 
...
#Remove NA
BM_EvictorFocal <- BM_EvictorFocal_WithNa %>% 
  filter(!is.na(WeightDiff))
...
So far, it looks like than in the sub treatment 
I can take the older animals as the focal 
...
ggplot(HeavierFocal_AgeDiff, aes(x = WeightDiff, y = AgeDiff)) +
  facet_grid(~DateType) +
  geom_hline(yintercept = 0) +
  geom_point(aes(shape = BS_Original, colour = Treatment)) + 
  geom_smooth(method = "lm")


ggplot(HeavierFocal_AgeDiff %>% 
         filter(!(Treatment == "Queen" & BS_Original == "Helper" & HeavierRef == 1)), aes(x = WeightDiff, y = AgeDiff)) +
  facet_grid(~DateType) +
  geom_hline(yintercept = 0) +
  geom_point(aes(shape = BS_Original, colour = Treatment)) + 
  geom_smooth(method = "lm")
...
#Correlation by breeding status
#could be driven by extreme points 
ggplot(HeavierFocal_AgeDiff, aes(x = WeightDiff, y = AgeDiff, color = Treatment)) +
  facet_grid(~DateType) +
  geom_hline(yintercept = 0) +
...
#Correlation by breeding status 
#Remove two sub heavier than queen 
#Correlation in queen treatment fall
ggplot(HeavierFocal_AgeDiff %>% 
         filter(!(Treatment == "Queen" & BS_Original == "Helper" & HeavierRef == 1)), aes(x = WeightDiff, y = AgeDiff, color = Treatment)) +
  facet_grid(~DateType) +
  geom_hline(yintercept = 0) +
  geom_point(aes(shape = BS_Original)) + 
  geom_smooth(method = "lm")
...
ggplot(OlderFocal_BMDiff , aes(x = AgeDiff, y = WeightDiff)) +
  facet_grid(~DateType) +
  geom_hline(yintercept = 0) +
  geom_point(aes(shape = BS_Original, colour = Treatment)) + 
  geom_smooth(method = "lm")
...
#Correlation by breeding status
#could be driven by extreme points 
ggplot(OlderFocal_BMDiff , aes(x = AgeDiff, y = WeightDiff, color = Treatment)) +
  facet_grid(~DateType) +
  geom_hline(yintercept = 0) +
  geom_point(aes(shape = BS_Original)) + 
  geom_smooth(method = "lm")
