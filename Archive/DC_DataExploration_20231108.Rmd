...
DC_Info_F <- read.csv("DC_Info_F_20240206.csv") %>% 
  ...
  #I ONLY REALIZE LATER THAT THIS HAD ALREADY BEEN DONE UNDER A DIFFERENT NAME SO THE INFORMATION IS DUPLICATED
  mutate(Eviction_Cat = case_when(EvictionPairingDayDiff <= 21 ~ "Rapid",
                                  EvictionPairingDayDiff >21 ~ "Slow", 
                                  TRUE ~ "None")) %>% 
  
  #READJUST EVICTION_GS
  #Just to be a bit more conservative and also based on data exploration 
  mutate(Eviction_GS = case_when(ParturitionEvictionDayDiff <=105 & ParturitionEvictionDayDiff >=87 ~ "Conception",
           TRUE ~ Eviction_GS))
...
SamplingCat <- DC_Info_F %>% 
  #RETAIN ONE ROW PER GROUP
  #Evictor and any lines of no eviction
  ...
  mutate(Sampling_Cat = case_when( Eviction_Cat == "Rapid" & Eviction_GS == "Conception" ~ "Rapid_Conc",
                                   Eviction_Cat == "Rapid" & Eviction_GS == "PreFirstConception" ~ "Rapid_PreFirst",
