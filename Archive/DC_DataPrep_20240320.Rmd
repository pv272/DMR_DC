...
Latest Parturition date must be checked still. It will be used to select samples pre-isolation.
...
LastParturition_OriginalGroup <- DC_PrePairingGroup %>%
  ...
  #ADD LITTER INFO
  #WHERE ONE RETAINS LATEST PARTURITION IN EACH GROUP
  left_join(., Litter_Info %>% 
              filter(Sex == "F") %>% 
              group_by(ConceptionColony) %>% 
              slice_max(ParturitionDate) %>% 
              ungroup() %>% 
              select(ConceptionColony,
                     ParturitionDate) %>% 
              rename(OriginalGroup = ConceptionColony,
                     OriginalGroup_LastParturitionDate = ParturitionDate)) %>% 
  distinct()
...
write.csv(DC_Info,"DC_Info_20240320.csv",row.names = FALSE)
...
#write csv
write.csv(DC_Info_F,"DC_Info_F_20240320.csv",row.names = FALSE)
...
write.csv(DC_Info_Group,"DC_Info_Group_20240320.csv",row.names = FALSE)
...
write.csv(DC_Info_Urine_F,"DC_Info_Urine_F_20240320.csv",row.names = FALSE)
