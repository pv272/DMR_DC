...
#DC Info Females
DC_Info_F <- read.csv("DC_Info_F_20240320.csv") %>% 
...
View(SampleList)
#829 samples before inventory 
#832 after inventory is joined


#write a csv that can be used to get sample out of the freezer 
write.csv(SampleList %>% 
            arrange(BoxID,
                    BoxPosition), "SampleList_20240320.csv", row.names = FALSE)
...
SampleList_Info <- SampleList %>% 
  ...
  mutate(SampleReason = fct_relevel(SampleReason, c("Isolation","PostPair10","PreEviction16","PostEviction14","RA","FirstConc")))
View(SampleList_Info)


#write csv
write.csv(SampleList_Info,"SampleList_Info_20240320.csv",row.names = FALSE)
...
write.csv(MissingSample_Summary,"MissingSample_Summary_20240321",row.names = FALSE)
