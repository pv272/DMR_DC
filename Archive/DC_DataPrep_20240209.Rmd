...
View(DC_Males)
#23 males 
#2 male are probably missing, unless some males have been reused
...
View(DC_EvictionDates)

anti_join(DC_ExperimentalGroup, DC_EvictionDates)
...
#Used gestation length of 88 to calculate trimester, 100 to -88 is conception
Eviction_GestationStatus <- Eviction_NextParturition_F %>% 
  
  #GESTATION STATUS SEMESTER
  gestationstatus(date = EvictionDate,gestationsplit = "trimester") %>% 
...
DC_Info <- 
  ...
  left_join(., DC_Parturition_Info_DCFirst %>% 
              select(AnimalID, 
                     ParturitionDate,
                     ParturitionColony) %>% 
              rename(FirstDCParturitionDate = ParturitionDate)) %>% 
  
    
  #ADD CONCEPTION DATE 
  #Take 88 days to make sure females are pregnant
  mutate(ConceptionDate = FirstDCParturitionDate - 88) %>% 
  ...
  mutate(ParturitionPairingDayDiff = FirstDCParturitionDate - PairingDate) 
  
  #ADD PARTURITION-EVICTION DELAY
  #mutate(ParturitionEvictionDayDiff = FirstDCParturitionDate - EvictionDate) %>% 
...
write.csv(DC_Info,"DC_Info_20240206.csv",row.names = FALSE)
...
Separate relevant info by sex and add sex specific info if relevant 
...
#write csv
write.csv(DC_Info_F,"DC_Info_F_20240206.csv",row.names = FALSE)
...
Generate a sampling category based on the data exploration performed in DC_DataExploration. This information will be useful to generate a dataset from which I can select the urine samples
...
SamplingCat <- DC_Info_F %>% 
  ...
  mutate(Sampling_Cat = relevel(Sampling_Cat, "Rapid_Conc", "Rapid_PreFirst", "Rapid")) %>% 
  
  arrange(Sampling_Cat)

View(SamplingCat)

unique(SamplingCat$Sampling_Cat)


#Add sampling category to 
```
...
DC_Info_Group <- DC_Info_F %>%
  ...
  mutate(CoBreeding = sum(!is.na(FirstDCParturitionDate) & ParturitionColony == ExperimentalGroup)) %>% 
    
    #ADD EARLIER FIRST PARTURITION 
    mutate(Earliest_FirstDCParturitionDate = min(FirstDCParturitionDate, na.rm = TRUE)) %>% 
    
    #ADD LATEST FIRST PARTURITION 
    mutate(Latest_FirstDCParturitionDate = max(FirstDCParturitionDate, na.rm = TRUE)) %>% 
    mutate(Latest_FirstDCParturitionDate = case_when(Earliest_FirstDCParturitionDate != Latest_FirstDCParturitionDate ~ Latest_FirstDCParturitionDate)) %>% 
  ...
  mutate(EvictionCategory = case_when(EvictionPairingDayDiff > 21 ~ "Late",
                                      TRUE ~ "Early")) %>% 
  ...
  ungroup() %>% 
    ...
    select(ExperimentalGroup,
         ...
         BreederCount,
         CoBreeding,
         Earliest_FirstDCParturitionDate,
         Latest_FirstDCParturitionDate,
...
write.csv(DC_Info_Group,"DC_Info_Group_20240206.csv",row.names = FALSE)
...
#write csv
write.csv(DC_Urine_F, "DC_Urine_F_20240206.csv", row.names = FALSE)
...
#write csv
write.csv(DC_ClosestWeight,"DC_ClosestWeight_20240206.csv",row.names = FALSE)
...
The end product should at the moment be sufficient for urine sample selection.
...
names(DC_Urine_F)
#1941 urine samples on females

View(DC_Info_F)

#DC info to join to urine 
#I need to tweak it so it includes the RS at aviction 
DC_Info_Urine <- DC_Info %>%
  ...
  left_join(.,DC_NextParturition_F %>% 
              ...
              rename(NextLitter_IndCount), by=c("AnimalID" = "AnimalID",
                                                 "UrineDate" = "Date"))
...
write.csv(DC_Info_Urine_F,"DC_Info_Urine_F_20240208.csv",row.names = FALSE)
