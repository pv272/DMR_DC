...
It could also be nice to add something about the type of eviction that follows whether that would be rapid or slow 
...
p <- ggplot(filtered_data, aes(x = interaction(SamplingPeriod,EvictCat_Gr), y = Conc)) +
  ...
  # ADD JITTERED POINTS COLORED BY EVICTION STATUS
  geom_point(aes(color = EvictionStatus), 
...
#PRINT PDF 
ppt <-   ppt %>% 
  print(paste("PostPair_SPF_",HormonesGroup_ToPlot[h],".pptx",sep = ""))
...
#PRINT PDF 
ppt <-   ppt %>% 
  print(paste("PostPair_SPF_D21_",HormonesGroup_ToPlot[h],".pptx",sep = ""))
...
View(Litter_Info %>% 
  filter(ParturitionColony %in% c("Lion","DevilsPeak","Tusker","WicklowWolf")) %>% 
...
View(DC_Info_F %>% 
  filter(ExperimentalGroup %in% c("Lion", "Tusker", "DevilsPeak",,"WicklowWolf")) %>% 
  select(ExperimentalGroup,
         AnimalID,
         BS_Original,
         EvictionStatus,
...
#Relative to isolation
Eviction <- DC_Info_F %>% 
    filter(ExperimentalGroup %in% c("Lion", "DevilsPeak","WicklowWolf")) %>% #Don't show eviction of Tusker since it happens sooooo late 
  distinct(ExperimentalGroup,
           EvictionPairingDayDiff) %>% 
  rename(Eviction = EvictionPairingDayDiff) %>% 
  filter(ExperimentalGroup == Group_ToPlot[i]) %>% 
...
#PRINT PDF 
ppt <-   ppt %>% 
  print(paste("PostPair_AllOthers_",HormonesGroup_ToPlot[h],".pptx",sep = ""))
