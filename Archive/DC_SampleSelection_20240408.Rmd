...
Sample 32233 (I assume) from SOF001 (Corbeau) 8 days before isolation seem to not have been labelled by mistake 32230. I realized this when extracting. I thus added this sample manually as I was missing pre-isolation sample for that individual 
...
PostEviction_14 <- DC_Info_Urine_F %>%
  #FILTER
  filter(UrineEviction_DayDiff >= 14) %>% #Here I should have taken the closest to 14 instead of necessarily smaller
  group_by(AnimalID) %>% 
  slice_min(UrineEviction_DayDiff) %>% 
  ungroup()
