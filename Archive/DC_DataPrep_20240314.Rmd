...
write.csv(DC_Info,"DC_Info_20240213.csv",row.names = FALSE)
...
#write csv
write.csv(DC_Info_F,"DC_Info_F_20240213.csv",row.names = FALSE)
...
write.csv(SamplingCat, "SamplingCat_20240314.csv", row.names = FALSE)
...
write.csv(DC_Info_Group,"DC_Info_Group_20240213.csv",row.names = FALSE)
...
#write csv
write.csv(DC_Urine_F, "DC_Urine_F_20240212.csv", row.names = FALSE)
...
#write csv
write.csv(DC_ClosestWeight,"DC_ClosestWeight_20240212.csv",row.names = FALSE)
...
#Names in order not to get lost when joining
names(DC_NextParturition_F)
names(DC_Dates_F_GestStat)
names(DC_Info)

#I shoudl have 3390 samples assuming no duplicated rows
...
DC_Info_Urine <- DC_Info %>%
  ...
  left_join(., DC_Dates_F_GestStat %>% 
              select(AnimalID, 
                     Date,
                     NextParturition_ParturitionDayDiff,
                     GestationStatus_Trimester), by=c("AnimalID" = "AnimalID",
                                                 "UrineDate" = "Date")) %>% 
  ...
  mutate(NextParturition_ParturitionDayDiff = case_when(AnimalID == "SOF006" ~ UrineDate - DOP,
                                                        TRUE ~ NextParturition_ParturitionDayDiff))

View(DC_Info_Urine)
#3193 samples instead of 3390 why? 
...
write.csv(DC_Info_Urine_F,"DC_Info_Urine_F_20240214.csv",row.names = FALSE)
