...
One basically wants to know which group had breeders of both sex at the start of experiment

How Am I going to do this without having to spend too much time on it? I think just 

- have a look in groups where there was no king removal 
- If queen and king were present at isolation
...
KingRemoved <- DC_Info_F %>% 
  ...
  ungroup()
View(KingRemoved)
#All groups were king was removed from were used to form the queen treatment. Some of these were also used to form the helper treatment 

#They were breeding at King's removal
...
# 5 original group, 4 subs were taken by Tinbergen 


#Group Comp of group where king was not removed
...
#Animals paired to form these group
NoKingRemoved_Paired <- Pairing_Info %>% 
  ...
  left_join(., DC_Info %>% 
              distinct(AnimalID,
                       Sex,
                       IsolationDate)) %>% 
  #ADD DEATH DATE ISOLATION DATE 
  mutate(DeathIsol_DayDiff = DeathDate - IsolationDate) %>% 
  
  #ONLY RETAIN LAST PAIRING INFO 
...
#What happened to these males
RuruMaags_MaleID <- Pairing_Info %>% 
  filter(PairingColony %in% c("The Maags", "RuRu")) %>% 
  left_join(Animal_Info) %>% 
  filter(Sex == "M") %>% 
  select(AnimalID, 
         DeathDate)
View(RuruMaags_MaleID)
...
View(DC_Info_F %>% 
       filter(OriginalGroup %in% c("The Maags", "RuRu")) %>% 
       mutate(LastParturition = OriginalGroup_LastParturitionDate - IsolationDate))

View(DC_Info_F)


#No king removed, helper treatment
NoKingRemoved_HT <- NoKingRemoved %>% 
  filter(Treatment == "Sub")
View(NoKingRemoved_HT)
#7 groups 
#That means that 6 groups came from the same original group as breeding group
...
#p(win) of breeder and non-breeder 
...
##Alternative 1: Separate queen treatment in two 
