...
DC_Info_F <- read.csv("DC_Info_F_20231106.csv") %>% 
...
#Relevant weight
DC_ClosestWeight <- read.csv("DC_ClosestWeight_20231106.csv")
names()


#Urine info 
#check what info is there
DC_Info_Urine_F <- read.csv("DC_Info_Urine_F_20231106.csv")
...
Lets's creat esampling categories. This is based on data exploration performed later in the current code, but useful to add here at the beginning of the code for subsequent filtering
...
Linear models show no difference in body mass at both isolation and pairing between treatments

Within the queen treatment, queen tend to be heavier at isolation and are significantly heavier at pairing than sub
...
Because at that stage, differences in body mass between the heavier animals and the lighter animals were greater in the sub treatment and variance in differences in BM was greater in the sub treatment that in the queen treatment. 

I don't think this was followed, so how do things look like now? 
Models suggest that there are no significant differences of delta BM across treatment, and this is all what matters. But I am not sure how appropriate the models are
...
because before this round, females from the queen treatment are on average 3 times older than females from the sub treatment. This is due to queens being much older than sub.This was not taken into account 

Q are significantly older than H
No difference in age in helpers across the two treatments
...
because the average age difference between females is 300 days in the sub treatment and over 3 years in the Queen treatment. This was not followed

The consequence is that the age difference between experimental subjects is significantly higher in the Q than in the Sub treatment
...
Eviction occurred in all but two group from the sub treatment (DevilsPeak, DOS = 18/07/2022, Lion = 02/03/2023) and in all but one group from the queen treatment (Tusker, DOS = 18/01/2022). 

Co-breeding only occur in Tusker from the queen treatment. Check that it is the case
...
16 evictions occured within 21 days (11S, 5 Q)
2 evictions occurred between 51 and 100 days
1 eviction between 101 and 150
3 eviction after 151
...
There are 12 groups where the evictor evicted within 21 days of pairing and around the time it conceived 
9x in Sub treatment
3x in Queen treatment (2x queen, 1x sub evictor).

Question? 
i) Had the evictee activated their reproductive axis? 
...
There are 3 groups where evictor evicteds shortly after eviction and before their first successful gestation 
- 1X Sub (Darling Brew, 116 days).
- 2X Queen treatment, evictor were sub in both cases (Chouffe - 140 days , BitBurger - 132 days).

Suggest no activation (Darling Brew?) prior to eviction or 1 luteal phase (Chouffe, BitBurger)

Question? 
i) Had the evictor activated their reproductive axis? or did they activate AFTER eviction when they succesfully conceived for the first time. 
ii) Had the evictee activated their reproductive axis? 
...
1x Sub treatment (Firestone)
That female died while giving birth 200 days post pairing. Thus should really be put in Cat 2

Question? 
i) Had the evictor activated their reproductive axis?
ii) Had the evictee activated their reproductive axis? 
...
2 queen treatment where the sub evicted the queen preg of their first litter long after pairing and around the time they conceived (Licher - 94 days, and Black label - 90 days pre-part). 
...
2x queen treatment where the sub evicted the queen long after pairing and before they got pregnant (Guinness - 207 days , Corbeau - 129 days prepart). In one case the queen was lactating. 
...
2x queen treatment (leffe, Wicklowolf) where the sub evicted the queen long after pairing and did not give birth after. In one case the queen was lactating
...
2x sub (Devils Peak, Lion)
1x Q (Tusker)

Tusker is the only group where we have co-breeding. 
